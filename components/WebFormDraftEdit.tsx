...
const WebFormDraftEdit: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  
  // --- TEMEL YAPILANDIRMA ---
  const [webFormInternalName, setWebFormInternalName] = useState('Taslak Kampanya #' + id);
  const [webFormUrlSuffixTR, setWebFormUrlSuffixTR] = useState('taslak-kampanya');
  const [webFormUrlSuffixEN, setWebFormUrlSuffixEN] = useState('draft-campaign');
  ...
  // Added isLinkEmpty helper function to fix compilation errors
    ...
    <div className="max-w-5xl mx-auto pb-20">
      <div className="mb-10 flex items-center justify-between">
        <div className="flex items-center gap-5">
          ...
          <div><h1 className="text-3xl font-bold text-gray-900 tracking-tight">Taslak Düzenle</h1><p className="text-gray-400 text-sm mt-0.5 font-medium">Form ID: #{id} • Taslak üzerindeki değişiklikleri yönetin.</p></div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => navigate('/web-formlar')} className="flex items-center gap-2 px-8 py-3 bg-blue-600 text-white rounded-2xl text-sm font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-100"><Save size={18} /> YAYINLA</button>
        </div>
      </div>
      ...
      <div className="space-y-12">
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/30 overflow-hidden">
          ...
          <div className="p-10 space-y-12">
            ...
            <section className="space-y-10">
              ...
              <div className="space-y-4"><div className="flex items-center justify-between px-1"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Özellik Maddeleri</label><button onClick={() => setBullets([...bullets, {tr: '', en: ''}])} className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-blue-100 transition-colors"><Plus size={14} /> Madde Ekle</button></div>{bullets.map((b, i) => (<div key={i} className="bg-gray-50/40 p-6 rounded-[2rem] border border-gray-100 space-y-4 relative group hover:border-blue-100 transition-all"><button onClick={() => setBullets(bullets.filter((_, idx) => idx !== i))} className="absolute top-4 right-4 text-red-300 hover:text-red-500 transition-colors"><Trash2 size={18} /></button><div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2"><div className="space-y-1.5"><div className="flex justify-between items-center px-1"><span className="text-[10px] font-black text-gray-400 tracking-tighter">TR</span><span className="text-[9px] font-bold text-gray-300">{b.tr.length}/80</span></div><input type="text" maxLength={80} value={b.tr} onChange={(e) => { const n = [...bullets]; n[i].tr = e.target.value; setBullets(n); }} className="w-full bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-semibold" /></div><div className="space-y-1.5"><div className="flex justify-between items-center px-1"><span className="text-[10px] font-black text-gray-400 tracking-tighter">EN</span><span className="text-[9px] font-bold text-gray-300">{b.en.length}/80</span></div><input type="text" maxLength={80} value={b.en} onChange={(e) => { const n = [...bullets]; n[i].en = e.target.value; setBullets(n); }} className="w-full bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-semibold" /></div></div></div>))}</div>
            </section>
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/30 overflow-hidden">
          ...
          <div className="p-10 space-y-12">
            <section className="space-y-10">
              ...
              <div className="space-y-0"><SectionLabel label="Tebrikler Üst Başlık" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={thanksTitleTR} onChange={(e) => setThanksTitleTR(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div><div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={thanksTitleEN} onChange={(e) => setThanksTitleEN(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div></div></div>
              <div className="space-y-0"><SectionLabel label="Tebrikler Alt Açıklama" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={thanksDescTR} onChange={(e) => setThanksDescTR(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div><div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={thanksDescEN} onChange={(e) => setThanksDescEN(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div></div></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                 <div className="bg-gray-50/50 p-8 rounded-[2rem] border border-gray-100 flex flex-wrap gap-12 h-full items-center">
                   <div className="flex items-center gap-4 group cursor-pointer select-none" onClick={() => setShowExpiryDate(!showExpiryDate)}>
                      ...
                      <div><p className={`text-sm font-bold transition-colors ${showExpiryDate ? 'text-emerald-700' : 'text-gray-700'}`}>Son Kullanma Tarihi</p><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-0.5">SAYFADA GÖSTER</p></div>
                   </div>
                   <div className="flex items-center gap-4 group cursor-pointer select-none" onClick={() => setShowValidClub(!showValidClub)}>
                      ...
                      <div><p className={`text-sm font-bold transition-colors ${showValidClub ? 'text-emerald-700' : 'text-gray-700'}`}>Geçerli Kulüp</p><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-0.5">SAYFADA GÖSTER</p></div>
                   </div>
                 </div>
                 <div className="space-y-4 h-full flex flex-col">
                    ...
                    <label className="flex-1 flex flex-col items-center justify-center min-h-[128px] border-2 border-dashed border-gray-100 rounded-[2rem] cursor-pointer hover:bg-gray-50 transition-all group overflow-hidden bg-white relative shadow-sm">
                      {thanksBottomImgUrl ? (<div className="relative w-full h-full group"><img src={thanksBottomImgUrl} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-gray-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[1px]"><Upload size={20} className="text-white" /></div></div>) : (<div className="text-center p-4"><Upload className="text-gray-200 mx-auto mb-1 group-hover:text-emerald-500 transition-colors" size={24} /><span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest uppercase">resim yükle</span></div>)}
            ...
            <section className="space-y-10">
              ...
              <div className="space-y-0"><SectionLabel label="Başlık Metni" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={stepsTitleTR} onChange={(e) => setStepsTitleTR(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-blue-300 transition-all shadow-sm" /></div><div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={stepsTitleEN} onChange={(e) => setStepsTitleEN(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-blue-300 transition-all shadow-sm" /></div></div></div>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">{stepsItems.map((item, idx) => (<div key={idx} className="flex gap-4 items-center bg-gray-50/50 p-6 rounded-[2.5rem] border border-gray-100 relative group transition-all hover:border-blue-200"><div className="w-12 h-12 bg-white border border-gray-100 rounded-[1.25rem] flex items-center justify-center text-sm font-black text-blue-600 shadow-sm shrink-0">{idx + 1}</div><div className="flex-1 space-y-4"><div className="grid grid-cols-2 gap-4"><input type="text" placeholder="TR Başlık" value={item.tr} onChange={(e) => { const n = [...stepsItems]; n[idx].tr = e.target.value; setStepsItems(n); }} className="bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-bold text-gray-700 outline-none focus:border-blue-300 transition-all" /><input type="text" placeholder="EN Başlık" value={item.en} onChange={(e) => { const n = [...stepsItems]; n[idx].en = e.target.value; setStepsItems(n); }} className="bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-bold text-gray-700 outline-none focus:border-blue-300 transition-all" /></div><div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4"><input type="text" placeholder="TR Alt Açıklama" value={item.descTr} onChange={(e) => { const n = [...stepsItems]; n[idx].descTr = e.target.value; setStepsItems(n); }} className="bg-white border border-gray-100 rounded-xl px-4 py-2 text-[11px] font-semibold text-gray-500 outline-none focus:border-emerald-300 transition-all italic" /><input type="text" placeholder="EN Alt Açıklama" value={item.descEn} onChange={(e) => { const n = [...stepsItems]; n[idx].descEn = e.target.value; setStepsItems(n); }} className="bg-white border border-gray-100 rounded-xl px-4 py-2 text-[11px] font-semibold text-gray-500 outline-none focus:border-emerald-300 transition-all italic" /></div></div>{stepsItems.length > 3 && (<button onClick={() => setStepsItems(stepsItems.filter((_, i) => i !== idx))} className="text-red-300 hover:text-red-500 transition-colors p-2 self-start mt-1"><Trash2 size={18} /></button>)}</div>))}</div>
              </div>
            </section>
            <section className="space-y-10">
              ...
              <div className="space-y-0"><SectionLabel label="Başlık Metni" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={infoTitleTR} onChange={(e) => setInfoTitleTR(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-purple-300 transition-all shadow-sm" /></div><div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={infoTitleEN} onChange={(e) => setInfoTitleEN(e.target.value)} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-purple-300 transition-all shadow-sm" /></div></div></div>
              <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12 pt-8 border-t border-gray-50">
              ...
              <div className="md:col-span-8 space-y-6 flex flex-col justify-center">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2"><div className="flex items-center justify-between px-1"><label className={`text-[11px] font-black uppercase tracking-widest transition-colors ${isLinkEmpty(activeAppTab) ? 'text-red-500' : 'text-gray-400'}`}>{activeAppTab === 'appsflyer' ? 'APPSFLYER URL' : `${activeAppTab.toUpperCase()} STORE URL`}</label></div><div className="relative"><input type="text" value={activeAppTab === 'appsflyer' ? appsFlyerLink : activeAppTab === 'ios' ? iosLink : activeAppTab === 'android' ? androidLink : huaweiLink} placeholder={activeAppTab === 'appsflyer' ? "https://app.appsflyer.com/..." : "https://..."} onChange={(e) => { const val = e.target.value; if (activeAppTab === 'appsflyer') setAppsFlyerLink(val); else if (activeAppTab === 'ios') setIosLink(val); else if (activeAppTab === 'android') setAndroidLink(val); else setHuaweiLink(val); }} className={`w-full border rounded-[1.5rem] px-6 py-4 text-sm font-semibold shadow-sm outline-none transition-all ${isLinkEmpty(activeAppTab) ? 'bg-red-50/30 border-red-200 text-red-700 focus:border-red-400 placeholder:text-red-300' : 'bg-white border-gray-100 text-emerald-600 focus:border-emerald-200'}`} /><LinkIcon className={`absolute right-6 top-1/2 -translate-y-1/2 transition-colors ${isLinkEmpty(activeAppTab) ? 'text-red-300' : 'text-gray-200'}`} size={18} /></div></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/30 overflow-hidden">
          ...
          <div className="p-10 space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">FORM_AREA_NAME</label><div className="relative"><input type="text" value={eventFormAreaName} onChange={(e) => setEventFormAreaName(e.target.value)} className="w-full bg-white border border-gray-100 rounded-[2rem] px-8 py-5 text-sm font-semibold text-gray-700 shadow-sm outline-none focus:border-pink-200" /><Layout className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-200" size={20} /></div></div><div className="space-y-2"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">EVENT CATEGORY</label><div className="relative"><input type="text" value={eventCategory} onChange={(e) => setEventCategory(e.target.value)} className="w-full bg-white border border-gray-100 rounded-[2rem] px-8 py-5 text-sm font-semibold text-gray-700 shadow-sm outline-none focus:border-pink-200" /><Activity className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-200" size={20} /></div></div></div>
            ...
            <div className="space-y-6 pt-6 border-t border-gray-50"><SectionLabel label="Event İsim Tanımları" /><div className="bg-white p-2 rounded-[2rem] space-y-8"><div className="flex items-center w-full border-b border-gray-100 mb-2">{eventSteps.map((step, idx) => (<button key={step.key} onClick={() => setActiveEventTab(idx)} className={`flex-1 pb-4 text-[11px] font-black uppercase tracking-widest transition-all duration-300 relative ${activeEventTab === idx ? 'text-pink-500' : 'text-gray-300 hover:text-gray-500'}`}>{idx + 1}. Adım {activeEventTab === idx && (<div className="absolute bottom-0 left-0 w-full h-0.5 bg-pink-500 animate-in slide-in-from-left-full duration-300"></div>)}</button>))}</div><div className="px-2 pt-2 animate-in fade-in slide-in-from-bottom-2 duration-300"><div className="space-y-8"><div className="border border-sky-400 rounded-xl px-6 py-4 bg-sky-50/30 flex items-start gap-3"><InfoIcon className="text-sky-500 shrink-0 mt-0.5" size={16} /><p className="text-xs font-semibold text-sky-800 leading-relaxed">{eventSteps[activeEventTab].description}</p></div><div className="space-y-2.5 px-1"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">EVENT NAME</label><div className="relative group"><input type="text" value={eventSteps[activeEventTab].value} onChange={(e) => eventSteps[activeEventTab].setter(e.target.value)} className="w-full bg-gray-50/50 border border-gray-100 rounded-[1.5rem] px-8 py-5 text-sm font-bold text-gray-700 outline-none focus:border-pink-200 focus:bg-white transition-all shadow-sm" /><Hash className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-200 group-focus-within:text-pink-200 transition-colors" size={20} /></div></div></div></div></div></div>
          </div>
        </div>
...
export default WebFormDraftEdit;