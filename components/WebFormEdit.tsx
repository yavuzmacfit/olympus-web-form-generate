...
import { LayoutPanelLeft, Plus, Save, ArrowLeft, Image as ImageIcon, Type, Trash2, Upload, CheckCircle2, Activity, Globe, Hash, QrCode, Smartphone, Link as LinkIcon, FileText, Info, MapPin, Building2, ChevronDown, X, Settings2, Layout, SlidersHorizontal, FileEdit, Link2, Languages, Search, AlertCircle, CheckSquare, Square, Info as InfoIcon, AlertTriangle } from 'lucide-react';
...
const WebFormEdit: React.FC = () => {
  ...
  const [selectedClubTypes, setSelectedClubTypes] = useState<string[]>(['Mac/ONE']);
  const [selectedClubs, setSelectedClubs] = useState<string[]>(['MAC/One Bebek']);
    ...
    <div className="max-w-5xl mx-auto pb-20 relative">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 fade-in duration-200">
            <div className="p-8 text-center space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-black text-gray-900">Emin misiniz?</h3>
                <p className="text-sm font-semibold text-gray-500 leading-relaxed px-4">
                  Yaptığınız değişiklikler yayındaki formun içeriğini değiştirecektir. Bu değişiklikten emin misiniz?
                </p>
              </div>
              <div className="flex flex-col gap-3">
                <button 
                  onClick={() => navigate('/web-formlar')}
                  className="w-full py-4 bg-blue-600 text-white rounded-2xl text-sm font-black tracking-wide hover:bg-blue-700 transition-all shadow-xl shadow-blue-100"
                >
                  EVET, DEĞİŞİKLİKLERİ YAYINLA
                </button>
                <button 
      ...
      <div className="mb-10 flex items-center justify-between">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Web Form Düzenle</h1>
            <p className="text-gray-400 text-sm mt-0.5 font-medium">Form ID: #{id} • Mevcut yapılandırmayı güncelleyin.</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button 
            ...
            disabled={!isDirty}
            className={`flex items-center gap-2 px-8 py-3 rounded-2xl text-sm font-bold transition-all shadow-xl ${
              isDirty 
                ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-200' 
                : 'bg-gray-100 text-gray-400 cursor-not-allowed shadow-none border border-gray-200'
            }`}
          >
            <Save size={18} /> DEĞİŞİKLİKLERİ YAYINLA
          </button>
        </div>
      </div>
      ...
      <div className="space-y-12">
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/30 overflow-hidden">
          ...
          <div className="p-10 space-y-10">
            <div className="space-y-2">
              <div className="flex justify-between items-center px-1 mb-1.5"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">WEB FORM İSMİ (INTERNAL)</label><span className="text-[9px] font-bold text-gray-300">{webFormInternalName.length}/40</span></div>
              <div className="relative">
                <input 
                  ...
                  readOnly
                  className="w-full bg-gray-50/50 border border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-bold text-gray-400 outline-none cursor-not-allowed select-none" 
                />
                <Type className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-200" size={18} />
              </div>
            </div>
            <div className="space-y-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center bg-gray-50/50 border border-gray-100 rounded-[1.5rem] px-6 py-4 cursor-not-allowed opacity-70">
                    <span className="text-[11px] font-bold text-gray-300 select-none whitespace-nowrap">.../tr/</span>
                    <input type="text" value={webFormUrlSuffixTR} readOnly className="flex-1 bg-transparent border-none text-sm font-bold text-red-600 outline-none pl-1 cursor-not-allowed" />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center bg-gray-50/50 border border-gray-100 rounded-[1.5rem] px-6 py-4 cursor-not-allowed opacity-70">
                    ...
                    <input type="text" value={webFormUrlSuffixEN} readOnly className="flex-1 bg-transparent border-none text-sm font-bold text-blue-600 outline-none pl-1 cursor-not-allowed" />
                    <Link2 className="text-gray-300 ml-2" size={18} />
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/30 overflow-hidden">
          ...
          <div className="p-10 space-y-12">
            ...
            <section className="space-y-10">
              ...
              <div className="space-y-0"><SectionLabel label="Ana Başlık Metni" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><LangIndicator lang="TR" current={leftTitleTR.length} max={60} /><input type="text" maxLength={60} value={leftTitleTR} onChange={(e) => { setLeftTitleTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-semibold text-gray-700 shadow-sm outline-none focus:border-red-200 transition-all" /></div><div className="space-y-2"><LangIndicator lang="EN" current={leftTitleEN.length} max={60} /><input type="text" maxLength={60} value={leftTitleEN} onChange={(e) => { setLeftTitleEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-semibold text-gray-700 shadow-sm outline-none focus:border-blue-200 transition-all" /></div></div></div>
              <div className="space-y-0"><SectionLabel label="Alt Açıklama Metni" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><LangIndicator lang="TR" current={leftDescTR.length} max={250} /><textarea maxLength={250} value={leftDescTR} onChange={(e) => { setLeftDescTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-semibold text-gray-700 shadow-sm outline-none min-h-[100px] focus:border-red-200 transition-all" /></div><div className="space-y-2"><LangIndicator lang="EN" current={leftDescEN.length} max={250} /><textarea maxLength={250} value={leftDescEN} onChange={(e) => { setLeftDescEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-semibold text-gray-700 shadow-sm outline-none min-h-[100px] focus:border-blue-200 transition-all" /></div></div></div>
              <div className="space-y-4"><div className="flex items-center justify-between px-1"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Özellik Maddeleri</label><button onClick={() => { setBullets([...bullets, {tr: '', en: ''}]); handleAnyChange(); }} className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-blue-100 transition-colors"><Plus size={14} /> Madde Ekle</button></div>{bullets.map((b, i) => (<div key={i} className="bg-gray-50/40 p-6 rounded-[2rem] border border-gray-100 space-y-4 relative group hover:border-blue-100 transition-all"><button onClick={() => { setBullets(bullets.filter((_, idx) => idx !== i)); handleAnyChange(); }} className="absolute top-4 right-4 text-red-300 hover:text-red-500 transition-colors"><Trash2 size={18} /></button><div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2"><div className="space-y-1.5"><div className="flex justify-between items-center px-1"><span className="text-[10px] font-black text-gray-400 tracking-tighter">TR</span><span className="text-[9px] font-bold text-gray-300">{b.tr.length}/80</span></div><input type="text" maxLength={80} value={b.tr} onChange={(e) => { const n = [...bullets]; n[i].tr = e.target.value; setBullets(n); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-semibold" /></div><div className="space-y-1.5"><div className="flex justify-between items-center px-1"><span className="text-[10px] font-black text-gray-400 tracking-tighter">EN</span><span className="text-[9px] font-bold text-gray-300">{b.en.length}/80</span></div><input type="text" maxLength={80} value={b.en} onChange={(e) => { const n = [...bullets]; n[i].en = e.target.value; setBullets(n); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-semibold" /></div></div></div>))}</div>
            </section>
            <section className="space-y-10"><div className="flex items-center gap-3 px-1"><div className="w-8 h-8 bg-orange-50 text-orange-500 rounded-lg flex items-center justify-center"><Layout size={18} /></div><h4 className="text-lg font-bold text-gray-800">Sağ Bölüm (Form Kartı)</h4></div><div className="space-y-0"><SectionLabel label="Form Kart Başlığı" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><LangIndicator lang="TR" current={formTitleTR.length} max={60} /><input type="text" maxLength={60} value={formTitleTR} onChange={(e) => { setFormTitleTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-semibold text-gray-700 shadow-sm outline-none focus:border-red-200 transition-all" /></div><div className="space-y-2"><LangIndicator lang="EN" current={formTitleEN.length} max={60} /><input type="text" maxLength={60} value={formTitleEN} onChange={(e) => { setFormTitleEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-semibold text-gray-700 shadow-sm outline-none focus:border-blue-200 transition-all" /></div></div></div>
              ...
              <div className="space-y-0"><SectionLabel label="Aksiyon Buton Metni" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="bg-[#fff1f1] border border-[#ffeded] rounded-[2rem] p-7 transition-all hover:bg-red-50/50"><LangIndicator lang="TR" current={buttonTextTR.length} max={30} /><input type="text" maxLength={30} value={buttonTextTR} onChange={(e) => { setButtonTextTR(e.target.value); handleAnyChange(); }} className="w-full bg-transparent border-none text-lg font-black text-red-700 tracking-wider text-center outline-none" /></div><div className="bg-[#f1f6ff] border border-[#edf3ff] rounded-[2rem] p-7 transition-all hover:bg-blue-50/50"><LangIndicator lang="EN" current={buttonTextEN.length} max={30} /><input type="text" maxLength={30} value={buttonTextEN} onChange={(e) => { setButtonTextEN(e.target.value); handleAnyChange(); }} className="w-full bg-transparent border-none text-lg font-black text-blue-700 tracking-wider text-center outline-none" /></div></div></div>
            </section>
          </div>
        </div>
          ...
          <div className="p-10 space-y-12">
            ...
            <section className="space-y-10">
              ...
              <div className="space-y-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={thanksTitleTR} onChange={(e) => { setThanksTitleTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div>
                  <div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={thanksTitleEN} onChange={(e) => { setThanksTitleEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div>
                </div>
              </div>
              ...
              <div className="space-y-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={thanksDescTR} onChange={(e) => { setThanksDescTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div>
                  <div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={thanksDescEN} onChange={(e) => { setThanksDescEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-emerald-300 transition-all shadow-sm" /></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                 <div className="bg-gray-50/50 p-8 rounded-[2rem] border border-gray-100 flex flex-wrap gap-12 h-full items-center">
                   <div className="flex items-center gap-4 group cursor-pointer select-none" onClick={() => { setShowExpiryDate(!showExpiryDate); handleAnyChange(); }}>
                   ...
                   <div className="flex items-center gap-4 group cursor-pointer select-none" onClick={() => { setShowValidClub(!showValidClub); handleAnyChange(); }}>
            ...
            <section className="space-y-10">
              ...
              <div className="space-y-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={stepsTitleTR} onChange={(e) => { setStepsTitleTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-blue-300 transition-all shadow-sm" /></div>
                  <div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={stepsTitleEN} onChange={(e) => { setStepsTitleEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-blue-300 transition-all shadow-sm" /></div>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Madde Ekle (Max 4, Min 3)</label>{stepsItems.length < 4 && (<button onClick={() => { setStepsItems([...stepsItems, {tr: '', en: '', descTr: '', descEn: ''}]); handleAnyChange(); }} className="text-blue-600 bg-blue-50 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-blue-100 transition-all"><Plus size={14} /> Adım Ekle</button>)}</div>
                      ...
                      <div className="flex-1 space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <input 
                            ...
                            value={item.tr} 
                            onChange={(e) => { const n = [...stepsItems]; n[idx].tr = e.target.value; setStepsItems(n); handleAnyChange(); }} 
                          ...
                          <input 
                            ...
                            value={item.en} 
                            onChange={(e) => { const n = [...stepsItems]; n[idx].en = e.target.value; setStepsItems(n); handleAnyChange(); }} 
                            className="bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-bold text-gray-700 outline-none focus:border-blue-300 transition-all" 
                          />
                        </div>
                        <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4">
                          <input 
                            ...
                            value={item.descTr} 
                            onChange={(e) => { const n = [...stepsItems]; n[idx].descTr = e.target.value; setStepsItems(n); handleAnyChange(); }} 
                          ...
                          <input 
                            ...
                            value={item.descEn} 
                            onChange={(e) => { const n = [...stepsItems]; n[idx].descEn = e.target.value; setStepsItems(n); handleAnyChange(); }} 
                            className="bg-white border border-gray-100 rounded-xl px-4 py-2 text-[11px] font-semibold text-gray-500 outline-none focus:border-emerald-300 transition-all italic" 
                          />
                        </div>
                      </div>
                      {stepsItems.length > 3 && (
                        <button onClick={() => { setStepsItems(stepsItems.filter((_, i) => i !== idx)); handleAnyChange(); }} className="text-red-300 hover:text-red-500 transition-colors p-2 self-start mt-1">
                          <Trash2 size={18} />
                        </button>
                      )}
                    </div>
            ...
            <section className="space-y-10">
              ...
              <div className="space-y-0">
                <SectionLabel label="Başlık Metni" />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={infoTitleTR} onChange={(e) => { setInfoTitleTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-purple-300 transition-all shadow-sm" /></div>
                  <div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={infoTitleEN} onChange={(e) => { setInfoTitleEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-purple-300 transition-all shadow-sm" /></div>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1"><label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Madde Ekle (Max 4, Min 3)</label>{infoItems.length < 4 && (<button onClick={() => { setInfoItems([...infoItems, {tr: '', en: ''}]); handleAnyChange(); }} className="text-purple-600 bg-purple-50 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-purple-100 transition-all"><Plus size={14} /> Madde Ekle</button>)}</div>
                <div className="grid grid-cols-1 gap-4">{infoItems.map((item, idx) => (<div key={idx} className="flex gap-4 items-center bg-gray-50/50 p-4 rounded-3xl border border-gray-100 relative group"><div className="w-10 h-10 bg-white border border-gray-100 rounded-2xl flex items-center justify-center text-sm font-black text-purple-600 shadow-sm shrink-0"><CheckCircle2 size={18} /></div><div className="flex-1 grid grid-cols-2 gap-4"><input type="text" placeholder="TR Bilgi" value={item.tr} onChange={(e) => { const n = [...infoItems]; n[idx].tr = e.target.value; setInfoItems(n); handleAnyChange(); }} className="bg-white border border-gray-100 rounded-xl px-4 py-2 text-xs font-semibold outline-none focus:border-purple-200" /><input type="text" placeholder="EN Bilgi" value={item.en} onChange={(e) => { const n = [...infoItems]; n[idx].en = e.target.value; setInfoItems(n); handleAnyChange(); }} className="bg-white border border-gray-100 rounded-xl px-4 py-2 text-xs font-semibold outline-none focus:border-purple-200" /></div>{infoItems.length > 3 && (<button onClick={() => { setInfoItems(infoItems.filter((_, i) => i !== idx)); handleAnyChange(); }} className="text-red-300 hover:text-red-500 transition-colors p-2"><Trash2 size={16} /></button>)}</div>))}</div>
              </div>
            </section>
            ...
            <section className="space-y-12">
               <div className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={downloadBtnTR} onChange={(e) => { setDownloadBtnTR(e.target.value); handleAnyChange(); }} className="w-full bg-red-50/30 border border-red-100 rounded-2xl px-6 py-5 text-sm font-black text-red-600 tracking-wider text-center outline-none focus:border-red-300 transition-all shadow-sm" /></div>
                     <div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={downloadBtnEN} onChange={(e) => { setDownloadBtnEN(e.target.value); handleAnyChange(); }} className="w-full bg-red-50/30 border border-red-100 rounded-2xl px-6 py-5 text-sm font-black text-red-600 tracking-wider text-center outline-none focus:border-red-300 transition-all shadow-sm" /></div>
                  </div>
               </div>
               <div className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="space-y-2"><LangIndicator lang="TR" /><input type="text" value={footerNoteTR} onChange={(e) => { setFooterNoteTR(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-gray-300 transition-all shadow-sm" /></div>
                     <div className="space-y-2"><LangIndicator lang="EN" /><input type="text" value={footerNoteEN} onChange={(e) => { setFooterNoteEN(e.target.value); handleAnyChange(); }} className="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm font-semibold outline-none focus:border-gray-300 transition-all shadow-sm" /></div>
                  </div>
               </div>
            </section>
                    ...
                    <div className="space-y-2.5 px-1">
                      ...
                      <div className="relative group">
                        <input 
                          ...
                          onChange={(e) => { eventSteps[activeEventTab].setter(e.target.value); handleAnyChange(); }} 
